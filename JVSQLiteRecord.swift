...
// TODO: remove old old commented code and the SQLite library after testing new framework
    ...
    private var sqlExpressions:(names:String, placeholders:String, pairs:String, values:[String], conditions:String){
        get{
            ...
            for case let (propertyName?, propertyValue) in introSpectionData.children{
                ...
                let possibleOptinalValue:Any? = propertyValue
                let unwrappedStringValue:String = String(describing:(possibleOptinalValue ?? ""))
                propertyValues.append(unwrappedStringValue)

                if let matchNames = matchFields{
                    if matchNames.contains(propertyName){
                        propertyMatches.append("\(propertyName) = \(unwrappedStringValue)")
                    }
                }
            }
